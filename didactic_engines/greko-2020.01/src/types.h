...
inline Pair operator*(const Pair& p, int x) { return Pair(x * p.mid, x * p.end); }

inline Pair operator*(double x, const Pair& p) { return Pair(int(x * p.mid), int(x * p.end)); }
inline Pair operator*(const Pair& p, double x) { return Pair(int(x * p.mid), int(x * p.end)); }

inline Pair operator+(const Pair& p1, const Pair& p2) { return Pair(p1.mid + p2.mid, p1.end + p2.end); }
inline Pair operator+(int x, const Pair& p) { return Pair(x + p.mid, x + p.end); }
inline Pair operator+(const Pair& p, int x) { return Pair(x + p.mid, x + p.end); }
