...
void VInitAttackData(void)
{
	...
	for (co = coWHITE; co <= coBLACK; co++) {
		...
		for (i = 0; i < 2; i++)	// Pawn left-attack and right-attack.
			VRay(1, s_argpvecPawn[co][i], c_argbfPc[pcPAWN][co]);
	}
	for (pc = pcKNIGHT; pc <= pcKING; pc++) {
		int	cIter;
		int const * pvec;

		cIter = c_argfSliding[pc] ? 7 : 1;
		for (pvec = s_argpvecPiece[pc]; *pvec; pvec++)
