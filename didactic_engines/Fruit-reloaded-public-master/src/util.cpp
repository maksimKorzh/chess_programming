...
static const char Version[] = "2.1";  // released version or build date if empty
...
char * my_version(char buffer[]) {

   ASSERT(buffer!=NULL);

   if (!my_string_empty(Version)) return (char *)Version;

   sprintf(buffer,"%02d/%02d/%02d",BUILD_DAY,BUILD_MONTH,BUILD_YEAR);
