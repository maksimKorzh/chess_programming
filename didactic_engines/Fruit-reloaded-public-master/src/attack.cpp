...
bool line_is_touched(const board_t * board, int from, int to, int piece, int target) {
   ...
   ASSERT(SQUARE_IS_OK(target));

   ASSERT(board->square[target]==Empty);
   ...
   if (PSEUDO_ATTACK(piece,delta)) {
      ...
      inc = DeltaIncLine[DeltaOffset+(to-from)];
      ASSERT(inc!=IncNone);
...
bool piece_attack_king(const board_t * board, int piece, int from, int king) {
   ...
   if (PIECE_IS_SLIDER(piece)) {
      ...
      for (delta_ptr = PieceDeltaDelta[code][DeltaOffset+(king-from)]; (delta=*delta_ptr) != DeltaNone; delta_ptr++) {
         ...
         to = from + delta;
         if (!SQUARE_IS_OK(to)) continue; // wrong from-to target
...
bool piece_is_touched(const board_t * board, int def, int from, int to, int piece, int colour) {

   int delta;
   int inc, sq;

   ASSERT(board!=NULL);
   ASSERT(SQUARE_IS_OK(def));
   ASSERT(SQUARE_IS_OK(from));
   ASSERT(SQUARE_IS_OK(to));
   ASSERT(piece_is_ok(piece));
   ASSERT(COLOUR_IS_OK(colour));

   ASSERT(board->square[def]==piece);
   ...
   if (PIECE_IS_PAWN(piece)) {
      ...
      inc = PAWN_MOVE_INC(colour);
      if (to-(inc-1) == def || to-(inc+1) == def) return true;
      ...
      if (PSEUDO_ATTACK(piece,delta)) {
         ...
         if (PIECE_IS_SLIDER(piece)) {
            ...
            do {
               ...
               if (sq == to) return true;
            } while (board->square[sq] == Empty || sq == from);

         } else {

            return true; // proved through PSEUDO_ATTACK()
         }
      }
   }
