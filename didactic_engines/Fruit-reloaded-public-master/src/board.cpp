...
bool board_is_ok(const board_t * board) {
   ...
   for (colour = 0; colour < ColourNb; colour++) {
      ...
      for (pos = 0; pos < size; pos++) {
         ...
         if ((board->pawn_bitboard[colour] & Bit[SQUARE_TO_64(sq)]) == 0) return false;
      }
   ...
   if (board->move != MoveNone && board->move != MoveNull && !move_is_ok(board->move)) return false;
   if (board->eval != ValueNone && !value_is_ok(board->eval)) return false;
   if (board->reduced != true && board->reduced != false) return false;
...
void board_clear(board_t * board) {
   ...
   for (colour = 0; colour < ColourNb; colour++) {
      board->pawn_bitboard[colour] = 0;
   }

   // chess960

   for (colour = 0; colour < ColourNb; colour++) {
      for (side = 0; side < SideNb; side++) {
         board->rook_square[colour][side] = SquareNone;
      }
   }

   // castle mask

   for (sq = 0; sq < SquareNb; sq++) board->castle_mask[sq] = 0xF; // TODO: remove static CastleMask in move_do.cpp ?
   ...
   for (colour = 0; colour < ColourNb; colour++) {
      ...
      for (sq_64 = 0; sq_64 < 64; sq_64++) {
         ...
         if (COLOUR_IS(piece,colour) && PIECE_IS_PAWN(piece)) {
            ...
            board->pawn_bitboard[colour] |= (U64(1) << sq_64);
         }
      }
