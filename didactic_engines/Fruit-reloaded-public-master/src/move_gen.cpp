...
void gen_promotes(list_t * list, const board_t * board, int turn) {
   ...
   LIST_CLEAR(list);
   inc = PAWN_MOVE_INC(turn);

   for (ptr = &board->pawn[turn][0]; (from=*ptr) != SquareNone; ptr++) {

      if (PAWN_RANK(from,turn) == Rank7) {
         to = from + inc;
         if (board->square[to] == Empty) {
            if (is_pinned(board,from,turn)) continue;
